...
class JishakuBase(commands.Cog):  # pylint: disable=too-many-public-methods
    ...
    def submit(self, ctx: commands.Context):
        ...
        if sys.version_info < (3, 7, 0):
            cmdtask = CommandTask(
                self.task_count, ctx, asyncio.Task.current_task()
    ...
    @commands.command(
        name="py_inspect", aliases=["pyi", "python_inspect", "pythoninspect"]
    )
    ...
    @commands.command(
        name="shell", aliases=["bash", "sh", "powershell", "ps1", "ps", "cmd"]
    )
    ...
    @commands.group(
        name="voice", aliases=["vc"], invoke_without_command=True, ignore_extra=False
    )
