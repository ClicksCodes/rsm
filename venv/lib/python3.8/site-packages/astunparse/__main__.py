...
def roundtrip_recursive(target, dump_tree=False):
    if os.path.isfile(target):
        ...
        print(
            "WARNING: skipping '%s', not a file or directory" % target, file=sys.stderr
        )
...
def main(args):
    ...
    parser.add_argument(
        "target", nargs="+", help="Files or directories to show roundtripped source for"
    )
    parser.add_argument(
        "--dump", type=bool, help="Show a pretty-printed AST instead of the source"
    )
