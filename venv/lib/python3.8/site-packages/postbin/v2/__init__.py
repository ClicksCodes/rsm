...
_HEADERS = {
    "User-Agent": f"PostBin (https://github.com/dragdev-studios/postbin)/{__version__})"
}
_FALLBACKS = [
...
class AsyncHaste:
    ...
    def close_session(self):
        ...
        loop.run_until_complete(
            self.session.close()
        )  # this could cause issues with other async apps
    ...
    async def _post(self, url, text, **kwargs):
        try:
            async with await self._get_session() as session:
                async with session.post(url, data=text, **kwargs) as response:
                    ...
                    if response.status not in [
                        200,
                        201,
                    ]:  # removed 202: That is processing, not complete.
            ...
            raise errors.OfflineServer(
                None, message="Exception while connecting - assuming dead host."
            ) from e
        ...
        *,
        ...
        try:
            res = await asyncio.wait_for(
                self._post(url + "/documents", text or self.text, headers=_HEADERS),
                timeout=timeout,
            )
...
async def postAsync(
    ...
    *,
    ...
    return await AsyncHaste().post(
        text, url=url, config=config, timeout=timeout, retries=retries
    )
