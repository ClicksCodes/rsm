...
def make_int_range(start, end, step=None):
    if any([s.startswith(("0", "-0")) for s in (start, end) if s not in ("0", "-0")]):
    ...
    r = xrange(start, end + 1, step) if start < end else xrange(start, end - 1, -step)
