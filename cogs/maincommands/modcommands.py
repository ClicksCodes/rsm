...
class Commands(commands.Cog):
    ...
    async def reasonHandler(self, m, dict, ctx):
        ...
        m = await ctx.send(
            embed=createEmbed(
                ...
                dict["prompt"]["desc"]
                + "\n"
                + f"Select {emojis['tick']} to provide no reason or {emojis['cross']} to cancel",
                dict["prompt"]["col"],
            )
        )
    ...
    async def warnPun(self, m, member, ctx, reason=None):
        ...
        try:
            if not ctx.author.guild_permissions.kick_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunWarn']} Looks like you don't have permissions",
                        "You need the `kick_members` permission to warn someone.",
                        colours["delete"],
                    )
                )
            if not ctx.guild.me.guild_permissions.kick_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunWarn']} Looks like I don't have permissions",
                        "I need the `kick_members` permission to warn someone.",
                        colours["delete"],
                    )
                )
        except:
        ...
        if reason == None:
            reason, m = await self.reasonHandler(
                ...
                {
                    "cancel": {
                        "title": f"{emojis['PunWarn']} Warning",
                        "desc": f"Warning cancelled.",
                        "col": colours["delete"],
                    },
                    "prompt": {
                        "title": f"{emojis['PunWarn']} Warning",
                        "desc": f"Please enter a reason for warning {member.mention}.",
                        "col": colours["create"],
                    },
                },
                ctx,
        ...
        if reason != None:
            try:
                await member.send(
                    embed=createEmbed(
                        f"{emojis['PunWarn']} Warning",
                        f"You were warned in {ctx.guild.name} {('for ' + reason) if reason is not False else 'with no reason provided'}.",
                        colours["edit"],
                    )
                )
            except:
            ...
            await m.edit(
                embed=createEmbed(
                    f"{emojis['PunWarn']} Warning",
                    f"User {member.mention} was successfully warned for {reason if reason is not False else 'No reason provided'}.",
                    colours["create"],
                )
            )
            await m.clear_reactions()
    ...
    async def kickPun(self, m, member, ctx, reason=None):
        ...
        try:
            if not ctx.author.guild_permissions.kick_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunKick']} Looks like you don't have permissions",
                        "You need the `kick_members` permission to kick someone.",
                        colours["delete"],
                    )
                )
            if not ctx.guild.me.guild_permissions.kick_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunKick']} Looks like I don't have permissions",
                        "I need the `kick_members` permission to kick someone.",
                        colours["delete"],
                    )
                )
        except:
        ...
        if reason == None:
            reason, m = await self.reasonHandler(
                ...
                {
                    "cancel": {
                        "title": f"{emojis['PunKick']} Kick",
                        "desc": f"Kick cancelled.",
                        "col": colours["delete"],
                    },
                    "prompt": {
                        "title": f"{emojis['PunKick']} Kick",
                        "desc": f"Please enter a reason for kicking {member.mention}.",
                        "col": colours["create"],
                    },
                },
                ctx,
        ...
        if reason != None:
            try:
                ...
                await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunKick']} Kick",
                        f"User {member.mention} was successfully kicked{' for' + str(reason) if reason is not False else ''}.",
                        colours["create"],
                    )
                )
            except:
                await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunKick']} Kick",
                        f"Something went wrong. I may not have permissions, or the user couldn't be kicked.",
                        colours["delete"],
                    )
                )
            await m.clear_reactions()
    ...
    async def banPun(self, m, member, ctx, reason=None):
        ...
        try:
            if not ctx.author.guild_permissions.ban_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunBan']} Looks like you don't have permissions",
                        "You need the `ban_members` permission to ban someone.",
                        colours["delete"],
                    )
                )
            if not ctx.guild.me.guild_permissions.ban_members:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunBan']} Looks like I don't have permissions",
                        "I need the `ban_members` permission to ban someone.",
                        colours["delete"],
                    )
                )
        except:
        ...
        if reason == None or len(reason) < 2:
            try:
                reason, m = await self.reasonHandler(
                    ...
                    {
                        "cancel": {
                            "title": f"{emojis['PunBan']} Ban",
                            "desc": f"Ban cancelled.",
                            "col": colours["delete"],
                        },
                        "prompt": {
                            "title": f"{emojis['PunBan']} Ban",
                            "desc": f"Please enter a reason for banning {member.mention}.",
                            "col": colours["create"],
                        },
                    },
                    ctx,
        ...
        if reason != None:
            try:
                if (
                    ...
                    or ctx.author.top_role.position <= member.top_role.position
                ):
                    int("return an error, quite clearly")
                try:
                    if reason is not False:
                        bm = await member.send(
                            embed=createEmbed(
                                f"{emojis['PunBan']} Banned",
                ...
                await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunBan']} Ban",
                        f"User {member.mention} was successfully banned{' for ' + str(reason) if reason is not False else ''}.",
                        colours["create"],
                    )
                )
            except:
                await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunBan']} Ban",
                        f"Something went wrong. I may not have permissions, or the user couldn't be banned.",
                        colours["delete"],
                    )
                )
                try:
                    bm.edit(
                        embed=createEmbed(
                            f"{emojis['PunBan']} Ban",
                            f"The ban in {ctx.guild.name} failed.",
                            colours["create"],
                        )
                    )
                except:
    ...
    async def softBanPun(self, m, member, ctx, reason=None):
        ...
        try:
            if not ctx.author.guild_permissions.ban_members:
                return await m.edit(
                    embed=self.createEmbed(
                        f"{emojis['PunSoftBan']} Looks like you don't have permissions",
                        "You need the `ban_members` permission to soft ban someone.",
                        colours["delete"],
                    )
                )
            if not ctx.guild.me.guild_permissions.ban_members:
                return await m.edit(
                    embed=self.createEmbed(
                        f"{emojis['PunSoftBan']} Looks like I don't have permissions",
                        "I need the `ban_members` permission to soft ban someone.",
                        colours["delete"],
                    )
                )
        except:
        ...
        if reason == None or len(reason) < 2:
            reason, m = await self.reasonHandler(
                ...
                {
                    "cancel": {
                        "title": f"{emojis['PunSoftBan']} Soft Ban",
                        "desc": f"Soft ban cancelled.",
                        "col": colours["delete"],
                    },
                    "prompt": {
                        "title": f"{emojis['PunSoftBan']} Soft Ban",
                        "desc": f"Please enter a reason for soft banning {member.mention}.",
                        "col": colours["create"],
                    },
                },
                ctx,
        ...
        if reason != None:
            try:
                ...
                await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunSoftBan']} Soft Ban",
                        f"User {member.mention} was successfully soft banned{' for' + str(reason) if reason is not False else ''}.",
                        colours["create"],
                    )
                )
            except:
    ...
    async def delHistoryPun(self, m, member, ctx, out=None, mod=None):
        ...
        try:
            if not ctx.author.guild_permissions.manage_messages:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunHistory']} Looks like you don't have permissions",
                        "You need the `manage_messages` permission to delete someone's history.",
                        colours["delete"],
                    )
                )
            if not ctx.guild.me.guild_permissions.manage_messages:
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['PunHistory']} Looks like I don't have permissions",
                        "I need the `manage_messages` permission to delete someone's history.",
                        colours["delete"],
                    )
                )
        except:
        ...
        if out == None:
            out, m = await self.intHandler(
                ...
                {
                    "cancel": {
                        "title": f"{emojis['PunHistory']} Delete History",
                        "desc": f"Delete history cancelled.",
                        "col": colours["delete"],
                    },
                    "prompt": {
                        "title": f"{emojis['PunHistory']} Delete History",
                        "desc": f"How many messages in this channel should I check? Max 100",
                        "col": colours["create"],
                    },
                    "default": 50,
                },
        ...
        if out != None:
            try:
                try:
                    ...
                    return await m.edit(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Delete History",
                            f"Something went wrong, I couldn't delete that many messages.",
                            colours["create"],
                        )
                    )
                if mod not in ["!", "not", "only"]:
                    deleted = await ctx.channel.purge(
                        limit=int(out), check=lambda m: m.author == member
                    )
                else:
                    deleted = await ctx.channel.purge(
                        limit=int(out), check=lambda m: m.author != member
                ...
                try:
                    await ctx.channel.send(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Delete History",
                            f"I deleted {len(deleted)} messages {'not ' if mod in ['only', 'not', '!'] else ''}by {member.mention}.",
                            colours["create"],
                        )
                    )
                    ...
                    await ctx.send(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Delete History",
                            f"I deleted {len(deleted)} of {member.mention}'s messages.",
                            colours["create"],
                        )
                    )
            except:
                await ctx.channel.send(
                    embed=createEmbed(
                        f"{emojis['PunHistory']} Delete History",
                        f"Something went wrong. I may not have permissions, or the users history couldn't be deleted.",
                        colours["delete"],
                    )
                )
            await m.clear_reactions()
        ...
        try:
            if not ctx.author.guild_permissions.manage_messages:
                return await ctx.send(
                    embed=createEmbed(
                        f"{emojis['slowmodeOn']} Slowmode",
                        "You need the `manage_messages` permission to set slowmode.",
                        colours["delete"],
                    ),
                    delete_after=10,
                )
            if not ctx.guild.me.guild_permissions.manage_messages:
                return await ctx.send(
                    embed=createEmbed(
                        f"{emojis['slowmodeOn']} Slowmode",
                        "I need the `manage_messages` permission to set slowmode.",
                        colours["delete"],
                    ),
                    delete_after=10,
                )
            if time > 21600:
                return await ctx.send(
                    embed=createEmbed(
                        f"{emojis['slowmodeOn']} Slowmode",
                        "Slowmode delay must be smaller than 21600.",
                        colours["delete"],
                    ),
                    delete_after=10,
                )
            if time < 0:
                return await ctx.send(
                    embed=createEmbed(
                        f"{emojis['slowmodeOn']} Slowmode",
                        "Slowmode delay must be bigger than 0.",
                        colours["delete"],
                    ),
                    delete_after=10,
                )
            ...
            return await ctx.send(
                embed=createEmbed(
                    f"{emojis['slowmodeOn'] if time > 0 else emojis['slowmodeOff']} Slowmode",
                    f"Slowmode time has been successfully set to {time}s.",
                    colours["create"],
                )
            )
        except:
            await ctx.send(
                embed=createEmbed(
                    f"{emojis['slowmodeOn'] if time > 0 else emojis['slowmodeOff']} Slowmode",
                    "An unknown error occurred and slowmode could not be set.",
                    colours["delete"],
                ),
                delete_after=10,
            )
    ...
    async def lockdown(self, lock, channel, ctx):
        ...
        if not ctx.author.guild_permissions.manage_channels:
            return await ctx.send(
                embed=createEmbed(
                    f"{emojis['lock']} Lock",
                    "You need the `manage_channels` permission to set lockdown.",
                    colours["delete"],
                ),
                delete_after=10,
            )
        if not ctx.guild.me.guild_permissions.manage_channels:
            return await ctx.send(
                embed=createEmbed(
                    f"{emojis['lock']} Lock",
                    "I need the `manage_channels` permission to set lockdown.",
                    colours["delete"],
                ),
                delete_after=10,
            )
        ...
        m = await ctx.send(
            embed=createEmbed(
                f"{emojis['lock']} Lock",
                f"Please wait as the channel gets {'un' if lock is None else ''}locked.",
                colours["edit"],
            )
        )
        ...
        return await m.edit(
            embed=createEmbed(
                f"{emojis['lock']} Lock",
                f"The channel is now {'un' if lock is None else ''}locked for everyone without `manage_messages` permission.",
                colours["create"],
            )
        )
    ...
    async def purgeChannel(self, m, ctx, out=None):
        ...
        try:
            if not ctx.author.guild_permissions.manage_messages:
                await ctx.send(
                    embed=createEmbed(
                        f"{emojis['PunHistory']} Looks like you don't have permissions",
                        "You need the `manage_messages` permission to purge a channel.",
                        colours["delete"],
                    )
                )
        ...
        if out == None:
            out, m = await self.intHandler(
                ...
                {
                    "cancel": {
                        "title": f"{emojis['PunHistory']} Purge Channel",
                        "desc": f"Purge Channel cancelled.",
                        "col": colours["delete"],
                    },
                    "prompt": {
                        "title": f"{emojis['PunHistory']} Purge Channel",
                        "desc": f"How many messages in this channel should I clear? Max 100",
                        "col": colours["create"],
                    },
                    "default": 50,
                },
        ...
        if out != None:
            try:
                try:
                    ...
                    return await m.edit(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Purge Channel",
                            f"Something went wrong, I couldn't delete that many messages.",
                            colours["create"],
                        )
                    )
                ...
                if out > 100:
                    out = 100
                deleted = await ctx.channel.purge(
                    limit=int(out), check=lambda m: not m.pinned
                )
                try:
                    await m.edit(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Purge Channel",
                            f"I deleted {len(deleted)-2} messages.",
                            colours["create"],
                        ),
                        delete_after=10,
                    )
                ...
                try:
                    await m.edit(
                        embed=createEmbed(
                            f"{emojis['PunHistory']} Purge Channel",
                            f"Something went wrong. I may not have permission to do that.",
                            colours["delete"],
                        ),
                        delete_after=10,
                    )
    ...
    async def punish(self, ctx, *member: discord.Member):
        ...
        e = discord.Embed(
            ...
            description=(
                f"Select punishment for {member.mention}:"
                + "\n"
                + (
                    "\n".join(
                        [
                            f"{ctx.bot.get_emoji(int(self.emojiids[t][0]))} | {self.emojiids[t][1]}"
                            for t in self.emojiids
                        ]
                    )
                )
            ),
        ...
        try:
            for emoji in self.emojiids:
                try:
                    await m.add_reaction(self.bot.get_emoji(self.emojiids[emoji][0]))
                except:
                    pass
            ...
            if o.name == "Stop":
                ...
                if o.name == "PunishWarn":
                    ...
                    await self.kickPun(m, member, ctx)
                elif o.name == "PunishBan":
                    await self.banPun(m, member, ctx)
                elif o.name == "PunishSoftBan":
                    await self.softBanPun(m, member, ctx)
                elif o.name == "PunishHistory":
                    await self.delHistoryPun(m, member, ctx)
                else:
                    print(o.name)
        except:
            await m.edit(embed=eClosed)
    ...
    async def userinfo(self, ctx, member: typing.Optional[discord.Member]):
        ...
        if member.is_avatar_animated() or str(member.discriminator).startswith("000"):
            flags.append("nitro")
        if member.bot:
            flags.append("bot")
        ...
        flagemojis = {
            "hypesquad_bravery": [
                "<:hypesquad_bravery:775783765930016789>",
                "Hypesquad Bravery",
            ],
            "early_supporter": [
            ...
            "bug_hunter_2": [
                "<:bug_hunter_2:775783766130950234>",
                "Bug Hunter level 2",
            ],
            "booster": ["<:Boosting1y:775783766131605545>", "Server Booster"],
            "rsm_developer": [
                "<:rsm_developer:775783766147858534>",
                "**RSM Developer**",
            ],
            "hypesquad_brilliance": [
                "<:hypesquad_brilliance:775783766152577095>",
                "Hypesquad Brilliance",
            ],
            "partner": ["<:partner:775783766178005033>", "Partner"],
            "hypesquad": ["<:hypesquad:775783766194126908>", "Hypesquad Events"],
            "bug_hunter_1": [
                "<:bug_hunter_1:775783766252847154>",
                "Bug Hunter level 1",
            ],
            "hypesquad_balance": [
                "<:hypesquad_balance:775783766303440937>",
                "Hypesquad Balance",
            ],
            "staff": ["<:staff:775783766383788082>", "Discord Staff"],
            "verified_bot_developer": [
                "<:verified_bot_developer:775783766425600060>",
                "Verified Bot Developer",
            ],
            "clicks_developer": [
                "<:clicks_developer:776140126156881950>",
                "Clicks Developer",
            ],
        ...
        for flag in flags:
            try:
                flagstring += flagemojis[flag][0] + f" | {flagemojis[flag][1]}\n"
            except:
                print(flag)
        ...
        embeds = {
            0: [
                ...
                f"**Status:** {emojis[member.status.name]} {member.status.name.capitalize() if member.status.name != 'dnd' else 'DND'}"
                + (" - Mobile" if member.mobile_status.name == "online" else ""),
        ...
        for r in [729064530310594601, 729762938411548694, 729762938843430952]:
        ...
        for _ in range(50):
            ...
            if reaction.name == "Left":
                p -= 1
            elif reaction.name == "Right":
                p += 1
            else:
                break
    ...
    async def nick(
        ...
        *,
        ...
        if not target:
            target = ctx.author
        if not name:
            for r in [729064531107774534, 729064530310594601]:
                await m.add_reaction(self.bot.get_emoji(r))
            await m.edit(
                embed=discord.Embed(
