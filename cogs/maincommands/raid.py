...
class Raid(commands.Cog):
    ...
    async def raid(self, ctx, toggle: typing.Optional[str]):
        if not (
            ...
            or (
                ...
                and ctx.author.guild_permissions.manage_server
            )
        ):
        ...
        raidInProgress = (
            True
            if discord.utils.get(ctx.guild.text_channels, name="rsm-raid-logs")
            else False
        )
    ...
    async def raidUI(self, ctx):
        ...
        for _ in range(50):
            ...
            await m.edit(
                embed=createEmbed(
                    title=f"{emojis['raidlock']} Raid in progress",
                    description=desc,
    ...
    async def timeban(self, m, ctx):
        ...
        mess = await ctx.bot.wait_for(
            "message", timeout=120, check=lambda message: message.author == ctx.author
        )
        if m.content.lower() != "cancel":
            try:
            ...
            for member in ctx.guild.members:
                d = datetime.strptime(
                    str(member.joined_at), "%Y-%m-%d %H:%M:%S.%f"
                ).strftime("%s.%f")
    ...
    async def ban(self, m, ctx):
        ...
        if m.content.lower() != "cancel":
            ...
            await m.edit(
                embed=createEmbed(
                    f"{emojis['raidlock']} Banned users",
                    f"{' '.join([mem.name for mem in mentions])}",
                    color=colours["create"],
                )
            )
    ...
    async def clear(self, m, ctx):
        ...
        if m.content.lower() != "cancel":
            ...
            a = await ctx.channel.purge(
                limit=500,
                check=lambda mes: mes.author.id in mentions and mes.id != m.id,
            )
            await m.edit(
                embed=createEmbed(
    ...
    async def toggleRaid(self, toggle, ctx):
        ...
        m = await ctx.send(
            embed=createEmbed(
                ...
                f"You are now {'entering' if toggle else 'leaving'} guild lockdown. {'We have to add a small delay between roles to ensure all roles are changed' if toggle else ''}",
                color=colours["delete"],
            )
        )
        if toggle == True:
            logChannel = discord.utils.get(
                ctx.guild.text_channels, name="rsm-raid-logs"
            )
            permsToStore = {}
            ...
            embed = createEmbed(
                f"{emojis['raidlock']} Raid in progress",
                f"This server is in guild lockdown.",
                color=colours["edit"],
            )
            ...
            if logChannel == None:
                ...
                try:
                    logChannel = await ctx.guild.create_text_channel(
                        "rsm-raid-logs", overwrites=overwrites
                    )
                except:
        ...
        if toggle == False:
            ...
            if logChannel == None:
                try:
                    await m.edit(
                        embed=createEmbed(
                            f"{emojis['raidlock']} Raid",
                            f"You are either not in a raid mode, or the log channel never got deleted. If you have a link that got sent in a different channel, please send just the link below. Otherwise, say `cancel`.",
                            color=colours["create"],
                        )
                    )
                    ...
                    if not match:
                        return await ctx.send(
                            emebed=createEmbed(
                                f"{emojis['raidlock']} Raid",
                                f"Got an invalid response. Please make sure to send only the link (can be without the `https://`, eg `https://hastebin.com/yazotefono` or `hastebin.com/yazotefono`)",
                                color=colours["delete"],
                            )
                        )
                except asyncio.TimeoutError:
            ...
            if not matchExists:
                ...
                match = re.match(
                    r"`ROLE PERMISSIONS: https:\/\/(.*)\/(.*)`(?:.|\n)*", m2.content
                )
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://{match.group(1)}/raw/{match.group(2)}"
                ) as resp:
                    data = json.loads(await resp.text())
                    try:
            ...
            await m.edit(
                embed=createEmbed(
                    f"{emojis['raidlock']} Raid",
                    f"The server has ended its lockdown.",
                    color=colours["create"],
                )
            )
