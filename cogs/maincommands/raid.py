...
class Raid(commands.Cog):
    ...
    async def raid(self, ctx, toggle: typing.Optional[str]):
        if not (ctx.author.guild_permissions.administrator or (ctx.author.guild_permissions.manage_channels and ctx.author.guild_permissions.manage_server)):
            return await ctx.send(
                embed=self.createEmbed(
                    f"{emojis['raidlock']} Looks like you don't have permissions",
                    "You need the `administrator` or both `manage_server` and `manage_channels` permissions to toggle raid.",
                    colours["delete"]),
                delete_after=10
            )
        ...
        if not raidInProgress:
            return await self.toggleRaid(True, ctx)
        elif toggle:
            return await self.toggleRaid(False, ctx)
        else:
            return await self.raidUI(ctx)
    ...
    async def raidrestore(self, ctx, url: typing.Optional[str]):
        ...
        if not url:
            return await ctx.send(
                embed=self.createEmbed(
                    f"{emojis['raidlock']} No link",
                    "Please provide a link to use for restoring your server.",
                    colours["delete"]),
                delete_after=10
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{url}") as resp:
                ...
                for key in data:
                    ...
                    try:
                        await ctx.guild.get_role(int(key)).edit(permissions=perms)
                    except Exception as e:
                        print(e)
        return await ctx.send(
            embed=self.createEmbed(
                f"{emojis['raidlock']} Your server is restored",
                "Everything should be back to normal. If it isn't, just make sure you used the right link.",
                colours["create"]
            ),
            delete_after=10
        )
    ...
    async def raidUI(self, ctx):
        ...
        for r in [729764053861400637, 729764053941223476, 729064530797133875, 729764062270980096, 777143043711172608, 729064530310594601]:
        ...
        for _ in range(50):
            ...
            if reaction is None:
                break
            elif reactionname == "punishban":
                await self.ban(m, ctx)
            elif reactionname == "punishsoftban":
                await self.timeban(m, ctx)
            elif reactionname == "purge":
                await self.purge(m, ctx)
            elif reactionname == "punishhistory":
                await self.clear(m, ctx)
            elif reactionname == "raid":
                await self.toggleRaid(False, ctx)
            else:
                break
    ...
    async def timeban(self, m, ctx):
        ...
        if not ctx.author.guild_permissions.ban_members:
            return await m.edit(embed=createEmbed(
                f"{emojis['raidlock']}",
                "You don't have permission to use this function",
                color=colours["delete"]
            ))
        await m.edit(embed=createEmbed(
            f"{emojis['raidlock']} Ban users",
            "Ban users that joined in the last `x` minutes. Type `cancel` to cancel.",
            color=colours["create"]
        ))
        mess = await ctx.bot.wait_for('message', timeout=120, check=lambda message: message.author == ctx.author)
        if m.content.lower() != "cancel":
            try:
                time = int(re.sub(r"[^0-9]*", "", str(mess.content))) * 60000
            except Exception as e:
                time = 1800000
            if (ctx.guild.me.joined_at - datetime.now()).total_seconds() < time:
                return await m.edit(embed=createEmbed(
                    f"{emojis['raidlock']} Banned users",
                    f"I joined before this point, and I may ban a lot of members!",
                    color=colours["create"]
                ))
            ...
            await m.edit(embed=createEmbed(
                f"{emojis['raidlock']} Banned users",
                f"Banned users that joined in the last {time/60000} minutes.",
                color=colours["create"]
            ))
    ...
    async def ban(self, m, ctx):
        ...
        createEmbed = self.createEmbed
        if not ctx.author.guild_permissions.ban_members:
            return await m.edit(embed=createEmbed(f"{emojis['raidlock']}", "You don't have permission to use this function", color=colours["delete"]))
        await m.edit(embed=createEmbed(
            f"{emojis['raidlock']} Ban users",
            "Mention all the users you want to ban. Type `cancel` to cancel.",
            color=colours["create"]
        ))
        mess = await ctx.bot.wait_for('message', timeout=120, check=lambda message: message.author == ctx.author)
        if m.content.lower() != "cancel":
    ...
    async def clear(self, m, ctx):
        ...
        createEmbed = self.createEmbed
        await m.edit(embed=createEmbed(
            f"{emojis['raidlock']} Clear messages by user",
            f"Mention all the users to clear from this chat. Type `cancel` to cancel.",
            color=colours["create"]
        ))
        mess = await ctx.bot.wait_for('message', timeout=120, check=lambda message: message.author == ctx.author)
        if m.content.lower() != "cancel":
            ...
            await m.edit(embed=createEmbed(
                f"{emojis['raidlock']} Cleared messages by users",
                f"Cleared {len(a)} messages by {' '.join([mem.name for mem in mess.mentions])}",
                color=colours["create"]
            ))
            await asyncio.sleep(3)
        return m
    ...
    async def purge(self, m, ctx):
        ...
        out, m = await self.intHandler(
            m,
            {
                ...
                "prompt": {
                    "title": f"{emojis['raidlock']} Purge Channel",
                    "desc": f"How many messages in this channel should I clear? Max 100. Type `cancel` to cancel.",
                    "col": colours["create"]
                },
                "default": 50
            },
        ...
        try:
            try:
                ...
                return await m.edit(
                    embed=createEmbed(
                        f"{emojis['raidlock']} Purge Channel",
            ...
            try:
                await m.edit(embed=createEmbed(f"{emojis['raidlock']} Purge Channel", f"I deleted {len(deleted)-2} messages.", colours["create"]), delete_after=10)
            except discord.ext.commands.errors.CommandInvokeError:
                await ctx.send(embed=createEmbed(f"{emojis['raidlock']} Purge Channel", f"I deleted {len(deleted)} messages.", colours["create"]), delete_after=10)
            except discord.errors.NotFound:
                await ctx.send(
                    embed=createEmbed(
                        f"{emojis['raidlock']} Purge Channel",
                        f"I deleted {len(deleted)} messages.",
                        colours["create"]
                    ),
                    delete_after=10
                )
    ...
    async def toggleRaid(self, toggle, ctx):
        ...
        m = await ctx.send(embed=createEmbed(
                f"{emojis['raidlock']} Raid",
                f"You are now {'entering' if toggle else 'leaving'} guild lockdown. "
                f"{'We have to add a small delay between roles to ensure all roles are changed' if toggle else ''}",
                color=colours["delete"]
            ))
        if toggle is True:
            ...
            for role in roles:
                ...
                if not role.permissions.manage_messages:
                    ...
                    try:
                        await role.edit(permissions=roleperms)
                    except Exception as e:
                        print(e)
                else:
                    roleperms = role.permissions
            ...
            if logChannel is None:
                ...
                try:
                    logChannel = await ctx.guild.create_text_channel('rsm-raid-logs', overwrites=overwrites)
                except Exception as e:
                    print(e)
                    logChannel = ctx.channel
            url = await postbin.postAsync(json.dumps(permsToStore))
            await logChannel.send(
                f"`ROLE PERMISSIONS: {url}`\n> Raid: Every role in the server without `manage_messages` has lost permission to send messages. "
                f"To end a raid, type `{ctx.prefix}raid off`. Role permissions have been stored, and can be restored when you run the raid off command. "
                f"After 30 days, the logs will be deleted.\nPlease do not send any messages in this chat.")
        ...
        if toggle is False:
            ...
            if logChannel is None:
                try:
                    await m.edit(embed=createEmbed(
                        f"{emojis['raidlock']} Raid",
                        f"You are either not in a raid mode, or the log channel never got deleted. If you have a link that got sent in a different channel, "
                        f"please send just the link below. Otherwise, say `cancel`.",
                        color=colours["create"]
                    ))
                    ...
                    if not match:
                        return await ctx.send(emebed=createEmbed(
                            f"{emojis['raidlock']} Raid",
                            f"Got an invalid response. Please make sure to send only the link (can be without the `https://`, eg "
                            f"`https://hastebin.com/yazotefono` or `hastebin.com/yazotefono`)",
                            color=colours["delete"]
                        ))
                except asyncio.TimeoutError:
                    return await m.edit(embed=createEmbed(
                        f"{emojis['raidlock']} Raid",
                        f"You are either not in a raid mode, or the log channel never got deleted. "
                        f"If you have a link that got sent in a different channel, please send just the link below. Otherwise, say `cancel`.",
                        color=colours["delete"]
                    ))
            try:
                match
