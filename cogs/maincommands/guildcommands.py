...
class GuildCommands(commands.Cog):
    ...
    async def guild(self, ctx):
        ...
        async def GraphUsers(ctx, m, g, final=False):
            ...
            ax.plot(
                joins_x_values,
                tuple(range(1, len(joins_x_values) + 1)),
                "k",
                lw=2,
            ...
            if final == False:
                await m.delete()
                m = await ctx.send(embed=e, file=image)
                return m
            else:
        ...
        async def genInfo(ctx, m, g, final=False):
            ...
            flags = {
                ...
                "us-west": ":flag_us:",
                "sidney": ":flag_au:",
                "vip-amsterdam": ":flag_nl:",
                "vip-us-east": ":flag_us:",
                "vip-us-west": ":flag_us:",
            }
            try:
                flag = flags[str(g.region)]
            except:
                flag = ""
        ...
        for x in range(0, 25):
            if 729762939023917086 not in [r.id for r in m.reactions]:
                await m.add_reaction(ctx.bot.get_emoji(729762939023917086))
            if 752214059159650396 not in [r.id for r in m.reactions]:
                await m.add_reaction(ctx.bot.get_emoji(752214059159650396))
            if 729064530310594601 not in [r.id for r in m.reactions]:
                await m.add_reaction(ctx.bot.get_emoji(729064530310594601))
            if page == 1:
                if 752570111281594509 in [r.id for r in m.reactions]:
                    await m.remove_reaction(ctx.bot.get_emoji(752570111281594509))
                if 729763053352124529 in [r.id for r in m.reactions]:
                    await m.remove_reaction(ctx.bot.get_emoji(729763053352124529))
                if 729066924943737033 in [r.id for r in m.reactions]:
                    await m.remove_reaction(ctx.bot.get_emoji(729066924943737033))
                if 729066518549233795 in [r.id for r in m.reactions]:
                    await m.remove_reaction(ctx.bot.get_emoji(729066518549233795))
            if page > 1:
                if 752570111281594509 not in [r.id for r in m.reactions]:
                    await m.add_reaction(ctx.bot.get_emoji(752570111281594509))
                if 729763053352124529 not in [r.id for r in m.reactions]:
                    await m.add_reaction(ctx.bot.get_emoji(729763053352124529))
                if 729066924943737033 not in [r.id for r in m.reactions]:
                    await m.add_reaction(ctx.bot.get_emoji(729066924943737033))
                if 729066518549233795 not in [r.id for r in m.reactions]:
                    await m.add_reaction(ctx.bot.get_emoji(729066518549233795))
            ...
            if reaction == None:
                ...
                page = 1
            elif reaction[0].emoji.name == "Graphs":
                page = 2
                ...
                page = 4
            elif reaction[0].emoji.name == "EmojisUpdate":
                page = 5
            else:
                break
            ...
            if page == 1:
                ...
                m = await GraphUsers(ctx, m, g)
            elif page == 3:
                m = await GraphRoles(ctx, m, g)
            elif page == 4:
                m = await GraphChannels(ctx, m, g)
            elif page == 5:
                m = await GraphEmojis(ctx, m, g)
            else:
                break
    ...
    async def setup(self, ctx):
        ...
        if not ctx.guild.me.guild_permissions.ban_members:
            missing.append("ban_members")
        if not ctx.guild.me.guild_permissions.manage_nicknames:
            missing.append("manage_nicknames")
        if not ctx.guild.me.guild_permissions.change_nickname:
            missing.append("change_nickname")
        if not ctx.guild.me.guild_permissions.view_audit_log:
            missing.append("view_audit_log")
        if not ctx.guild.me.guild_permissions.read_messages:
        ...
        if not ctx.guild.me.guild_permissions.add_reactions:
            missing.append("add_reactions")
        if not ctx.guild.me.guild_permissions.use_external_emojis:
            missing.append("use_external_emojis")
        if len(missing):
            try:
                await ctx.send(
                    f"Your server now has no settings, but I am missing permissions. Please make sure I have: {', '.join(['`' + p + '`' for p in missing])} and you'll be on your way!"
                )
            except:
    ...
    async def role(self, ctx, *, r: typing.Union[discord.Role, discord.Member]):
        if type(r) == discord.Role:
            ...
            async def genPage(ctx, m, page, final=False):
                await m.edit(
                    embed=discord.Embed(
                        ...
                        description=page
                        + n
                        + "".join([thing for thing in perms[page]]),
                        color=colours["create"] if not final else colours["delete"],
                    )
                )
                return m
            ...
            for re in [
                ...
                await m.add_reaction(ctx.bot.get_emoji(re))
            for _ in range(0, 50):
                page = min(4, max(0, page))

                pages = {1: "Server", 2: "Messages", 3: "Members", 4: "Voice"}
                if page == 0:
                    m = await genInfo(ctx, m)
                else:
                    m = await genPage(ctx, m, pages[page])
                ...
                try:
                    reaction = await ctx.bot.wait_for(
                        "reaction_add",
                        timeout=60,
                        check=lambda r, user: r.message.id == m.id
                        and user == ctx.author,
                    )
                except asyncio.TimeoutError:
                    break
                ...
                elif reaction.name == "GeneralRole":
                    page = 0
                elif reaction.name == "ServerRole":
                    page = 1
                elif reaction.name == "MessagesRole":
                    page = 2
                elif reaction.name == "MembersRole":
                    page = 3
                elif reaction.name == "VoiceRole":
                    page = 4
                else:
                    break
            ...
            pages = {1: "Server", 2: "Messages", 3: "Members", 4: "Voice"}
            if page == 0:
                m = await genInfo(ctx, m, True)
            else:
                m = await genPage(ctx, m, pages[page], True)
            ...
            if len(ctx.guild.roles) == 0:
                await ctx.send(
                    embed=discord.Embed(
                        title="Your server has no roles", color=colours["delete"]
                    )
                )
            roles = r.roles
            ...
            for reaction in [
                ...
                await m.add_reaction(ctx.bot.get_emoji(reaction))
            reaction = None
            ...
            for _ in range(0, 50):
                ...
                for x in range(0, 10):
                    if (
                        rolesOnPage <= x if page == pages - 1 else rolesOnPage < x
                    ) and (f"{x}_" in reactions):
                        await m.clear_reaction(bot.get_emoji(numbers[x]))
                    if (rolesOnPage >= x + (0 if page != pages - 1 else 1)) and (
                        f"{x}_" not in reactions
                    ):
                        await m.add_reaction(bot.get_emoji(numbers[x]))

                desc = f"Page {page+1} of {pages}\n"
                for x in range(
                    (page * 10),
                    (page * 10) + (rolesOnPage + (0 if page == pages - 1 else 1)),
                ):
                    desc += f"{bot.get_emoji(tick if groles[x] in roles else cross)}{bot.get_emoji(PILNumbers[str(groles[x] in roles)][x%10])} {groles[x]}\n"
                await m.edit(
                    embed=discord.Embed(
                        title="Roles", description=desc, color=colours["create"]
                    )
                )
                ...
                if reaction.name == "Left":
                    ...
                    page += 1
                elif reaction.name == "Cross":
                    break
                else:
                    try:
                        roleToChange = ctx.guild.get_role(
                            groleIDs[(page * 10) + int(reaction.name[:1])]
                        )
                        if ctx.guild.me.top_role.position <= roleToChange.position:
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} I can't do that",
                                    description="I can't change that role",
                                    colour=colours["delete"],
                                ),
                                delete_after=5,
                            )
                        elif ctx.author.top_role.position <= roleToChange.position:
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} You can't do that",
                                    description="You can't change that role",
                                    colour=colours["delete"],
                                ),
                                delete_after=5,
                            )
                        elif roleToChange.position == 0:
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} You can't do that",
                        ...
                        elif roleToChange.managed:
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} You can't do that",
                                    description="This role is for a bot",
                                    colour=colours["delete"],
                                ),
                                delete_after=5,
                            )
                            ...
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} Looks like you don't have permissions",
                                    description="You need the `manage_roles` permission to change roles.",
                                    colour=colours["delete"],
                                ),
                                delete_after=5,
                            )
                            ...
                            await ctx.send(
                                embed=discord.Embed(
                                    title=f"{emojis['PunWarn']} Looks like I don't have permissions",
                                    description="I need the `manage_roles` permission to change roles.",
                                    colour=colours["delete"],
                                ),
                                delete_after=5,
                            )
                        else:
                            if roleToChange in r.roles:
                                await r.remove_roles(roleToChange)
                            else:
                                await r.add_roles(roleToChange)
            ...
            await m.edit(
                embed=discord.Embed(
                    title="Roles", description=desc, color=colours["delete"]
                )
            )
            await m.clear_reactions()
    ...
    async def roleall(self, ctx, *, s: typing.Optional[str]):
        ...
        roles = {
            r.name: r.id
            for r in list(reversed([r for r in ctx.guild.roles if not r.managed]))[1:]
        }
        search = s.split(" ")[:1][0].lower()
        n = "\n"
        valid = {}
        for role, ID in roles.items():
            if search in role.lower() or role.lower() in search:
                valid[role] = ID
        v = {}
        x = 0
        es = [
        ...
        for key, value in valid.items():
            ...
            if x > 10:
                break
            v[key] = [
                f"{self.bot.get_emoji(es[x-1])} | {ctx.guild.get_role(value).mention}",
                value,
            ]
        ...
        for emoji in [729064531107774534, 729064530310594601, 751161404442279966]:
            await m.add_reaction(self.bot.get_emoji(emoji))
        reaction = None
        try:
            reaction = await ctx.bot.wait_for(
                "reaction_add",
        ...
        for member in ctx.guild.members:
            if (member.bot and bot) or (not member.bot and human):
                try:
                    if to == "+":
                        await member.add_roles(r, reason="Role all with RSM")
                    elif to == "-":
                        await member.remove_roles(r, reason="Role all with RSM")
                except Exception as e:
                    print(e)
                await asyncio.sleep(1)
        await m.edit(
            embed=discord.Embed(
                title=f"{emojis['role_create']} Complete", color=colours["create"]
            )
        )
    ...
    async def viewfrom(self, ctx, m: typing.Optional[discord.Member]):
        ...
        for cat in g.categories:
            ...
            visible[cat] = []
        for c in g.channels:
            server[c.category if c.category else 0].append(c)
        ...
        for cat in server:
            for channel in server[cat]:
                if channel.type in [
                    ...
                    if (m.permissions_in(channel).read_messages) or (
                        channel.type is discord.ChannelType.voice
                        and m.permissions_in(channel).connect
                    ):
        ...
        for r in [729762938411548694, 729762938843430952, 729064530310594601]:
            await mess.add_reaction(ctx.bot.get_emoji(r))
        for _ in range(0, 50):
            ...
            try:
                reaction = await ctx.bot.wait_for(
                    "reaction_add",
                    timeout=60,
                    check=lambda r, user: r.message.id == mess.id
                    and user == ctx.author,
                )
            except asyncio.TimeoutError:
                break
            ...
            try:
                await mess.remove_reaction(reaction, ctx.author)
            except:
                pass
