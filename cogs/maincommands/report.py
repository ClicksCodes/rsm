...
class Report(commands.Cog):
    ...
    async def stafflog(self, ctx, channel: typing.Optional[typing.Union[discord.TextChannel, int]]):
        ...
        if ctx.guild.id == channel.guild.id:
            with open(f"data/guilds/{ctx.guild.id}.json", 'r') as entry:
                ...
                try: entry["log_info"]["staff"] = channel.id
                except KeyError: entry["log_info"]["staff"] = None
    ...
    async def report(self, ctx, *, content=""):
        ...
        if isinstance(ctx.channel, discord.channel.DMChannel):
            ...
            if not len(mutuals): return await m.edit(embed=discord.Embed(
                ...
                for _ in range(50):
                    ...
                    s = ""
                    for x in range(len(chunk)): s += f"{emojis[str(x+1)]} {chunk[x]}\n"
                    await m.edit(embed=discord.Embed(
        ...
        with open(f"data/guilds/{guild.id}.json", 'r') as entry:
            entry = json.load(entry)
            try: logchan = self.bot.get_channel(entry['log_info']['staff'])
            except: logchan = None
        if not logchan:
            return await m.edit(embed=discord.Embed(
        ...
        if not content:
            ...
            try: msg = await ctx.bot.wait_for('message', timeout=60, check=lambda m : m.author.id == ctx.author.id and m.channel.id == ctx.channel.id)
