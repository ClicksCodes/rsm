...
class Messages(commands.Cog):
    ...
    async def on_message(self, message: discord.message):
        if isinstance(message.channel, discord.channel.DMChannel):
            return
        guild = message.guild
        if message.mention_everyone and self.is_logging(
            guild,
            channel=message.channel,
            member=message.author,
            eventname="everyone_here",
        ):
            ...
            edited = (
                humanize.naturaltime(datetime.utcnow() - message.edited_at)
                if message.edited_at
                else "Never"
            )
            e = discord.Embed(
                title=emojis["everyone_ping"]
                + f" {'Here' if '@here' in message.content else 'Everyone'} pinged",
                description=(
        ...
        if len(message.role_mentions) and self.is_logging(
            guild, channel=message.channel, member=message.author, eventname="roles"
        ):
            log = self.get_log(message.guild)
            if not log:
                return
        ...
        if len(message.mentions) > 4 and self.is_logging(
            guild,
            channel=message.channel,
            member=message.author,
            eventname="mass_mention",
        ):
            log = self.get_log(message.guild)
            if not log:
                return
    ...
    async def on_bulk_message_delete(self, messages):
        ...
        audit = await get_alog_entry(
            message.channel, type=discord.AuditLogAction.message_bulk_delete
        )
    ...
    async def on_guild_channel_pins_update(
        self, channel: discord.TextChannel, last_pin: datetime
    ):
        ...
        audit = await get_alog_entry(
            channel,
            type=discord.AuditLogAction.message_pin,
            check=lambda l: l.extra.channel.id == channel.id,
        )
        if not audit:
            return NotLogging(
        ...
        e = discord.Embed(
            title=emojis["pinned"]
            + f" Message {'un' if not message.pinned else ''}pinned",
            description=(
