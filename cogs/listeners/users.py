...
class Users(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        if not self.is_logging(member.guild, member=member, eventname="member_join"):
            return
        e = discord.Embed(
            title=(emojis["bot_join"] if member.bot else emojis["join"])
            + f" Member Joined",
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if audit.target.id == member.id:
            return
        e = discord.Embed(
            title=(emojis["bot_leave"] if member.bot else emojis["leave"])
            + f" Member Left",
    ...
    async def on_member_unban(self, guild, member: discord.Member):
        ...
        audit = await get_alog_entry(
            guild.channels[0], type=discord.AuditLogAction.unban
        )
        e = discord.Embed(
        ...
        return await self.log(
            ...
            occurredAt=round(time.time()),
            guild=guild.id,
            content={"username": member.id, "unbannedBy": audit.user.id},
        )
    ...
    async def on_voice_state_update(self, m, before, after):
        if before.channel == None and after.channel != None:
            ...
            return await self.log(
                ...
                content={"username": m.id, "channel": after.channel.name},
            )
            ...
            return await self.log(
                ...
                content={"username": m.id, "channel": before.channel.name},
            )
        elif (
            before.channel != None
