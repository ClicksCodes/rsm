...
class Guild(commands.Cog):
    ...
    async def on_webhooks_update(self, channel):
        if not self.is_logging(channel.guild, channel=channel, member=None, eventname="webhook_create"):
            return
        ...
        if auditCreate.created_at > max(auditUpdate.created_at, auditDelete.created_at):
            audit = auditCreate
            t = "create"
        elif auditUpdate.created_at > max(auditCreate.created_at, auditDelete.created_at):
            audit = auditUpdate
            t = "update"
        elif auditDelete.created_at > max(auditUpdate.created_at, auditCreate.created_at):
            audit = auditDelete
            t = "delete"
        else:
            return
    ...
    async def on_guild_channel_create(self, channel):
        c_type = str(channel.type).split('.')[-1]
        if c_type == "category":
            if not self.is_logging(channel.guild, channel=None, member=None, eventname="channel_create"):
                return
        else:
            if not self.is_logging(channel.guild, channel=channel, member=None, eventname="channel_create"):
                return
    ...
    async def on_guild_channel_delete(self, channel):
        c_type = str(channel.type).split('.')[-1]
        if c_type == "category":
            if not self.is_logging(channel.guild, channel=None, member=None, eventname="channel_delete"):
                return
        else:
            if not self.is_logging(channel.guild, channel=channel, member=None, eventname="channel_delete"):
                return
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        if c_type in ["text", "news", "voice", "store"]:
            changes = []
            if (before.name != after.name) and self.is_logging(after, channel=after, eventname="channel_title_update"):
                changes.append("TitleUpdate")
            if c_type in ["text"]:
                if before.is_nsfw() and not after.is_nsfw() and self.is_logging(after.guild, channel=after, eventname="nsfw_update"):
                    changes.append("nsfw_off")
                elif not before.is_nsfw() and after.is_nsfw() and self.is_logging(after.guild, channel=after, eventname="nsfw_update"):
                    changes.append("nsfw_on")
                if (before.topic != after.topic) and not (before.topic == "" and after.topic is None) and self.is_logging(
                    after.guild,
                    channel=after,
                    eventname="channel_desc_update"
                ):
    ...
    async def on_invite_create(self, invite: discord.Invite):
        if not isinstance(invite.guild, discord.Guild):
            return
        if not self.is_logging(invite.channel.guild, channel=invite.channel, member=None, eventname="invite_create"):
            return
    ...
    async def on_invite_delete(self, invite: discord.Invite):
        if not isinstance(invite.guild, discord.Guild):
            return
        if not self.is_logging(invite.guild, channel=invite.channel, member=None, eventname="invite_create"):
            return
