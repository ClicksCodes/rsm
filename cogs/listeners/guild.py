...
class Guild(commands.Cog):
    ...
    async def on_guild_update(self, before, after):
        changes = []
        if (before.name != after.name) and self.is_logging(
            after, eventname="name_changed"
        ):
            changes.append("name_change")
        if (
            before.verification_level.name != after.verification_level.name
        ) and self.is_logging(after, eventname="mod_changed"):
            changes.append("mod_update")
        if (before.icon_url != after.icon_url) and self.is_logging(
            after, eventname="icon_update"
        ):
        ...
        if len(changes):
            ...
            e = discord.Embed(
                ...
                if "mod_update" in changes
                else ""
                f"**Icon:** [Before]({before.icon_url}) > [After]({after.icon_url})\n"
                if "icon_update" in changes
                else "" f"**Changed by:** {audit.user.mention}",
            ...
            return await self.log(
                ...
                content={
                    ...
                    "modLevel": [
                        before.verification_level.name,
                        after.verification_level.name,
                    ],
    ...
    async def on_webhooks_update(self, channel):
        if not self.is_logging(
            channel.guild, channel=channel, member=None, eventname="webhook_create"
        ):
            return
        auditCreate = await get_alog_entry(
            channel, type=discord.AuditLogAction.webhook_create
        )
        auditUpdate = await get_alog_entry(
            channel, type=discord.AuditLogAction.webhook_update
        )
        auditDelete = await get_alog_entry(
            channel, type=discord.AuditLogAction.webhook_delete
        )

        if auditCreate.created_at > max(auditUpdate.created_at, auditDelete.created_at):
            audit = auditCreate
            t = "create"
        elif auditUpdate.created_at > max(
            auditCreate.created_at, auditDelete.created_at
        ):
            audit = auditUpdate
            t = "update"
        elif auditDelete.created_at > max(
            auditUpdate.created_at, auditCreate.created_at
        ):
            audit = auditDelete
            t = "delete"
        else:
        ...
        if t == "create":
            ...
            return await self.log(
                ...
                content={"username": audit.user.id},
            )
        elif t == "update":
    ...
    async def on_guild_channel_create(self, channel):
        ...
        if c_type == "category":
            if not self.is_logging(
                channel.guild, channel=None, member=None, eventname="channel_create"
            ):
                return
        else:
            if not self.is_logging(
                channel.guild, channel=channel, member=None, eventname="channel_create"
            ):
                return
        audit = await get_alog_entry(
            channel, type=discord.AuditLogAction.channel_create
        )
        log = self.get_log(channel.guild)
        e = discord.Embed(
            title=emojis[
            ...
            + " "
            + ("Channel" if c_type != "category" else "Category")
            + " Created",
        ...
        return await self.log(
            ...
            occurredAt=round(time.time()),
            guild=channel.guild.id,
            content={"username": audit.user.id, "channel": channel.id, "type": c_type},
        )
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        if c_type == "category":
            if not self.is_logging(
                channel.guild, channel=None, member=None, eventname="channel_delete"
            ):
                return
        else:
            if not self.is_logging(
                channel.guild, channel=channel, member=None, eventname="channel_delete"
            ):
                return
        audit = await get_alog_entry(
            channel, type=discord.AuditLogAction.channel_delete
        )
        log = self.get_log(channel.guild)
        e = discord.Embed(
            title=emojis[
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        if c_type in ["text", "news", "voice", "store"]:
            changes = []
            if (before.name != after.name) and self.is_logging(
                after, channel=after, eventname="channel_title_update"
            ):
                changes.append("TitleUpdate")
            if c_type in ["text"]:
                if (
                    before.is_nsfw()
                    and not after.is_nsfw()
                    and self.is_logging(
                        after.guild, channel=after, eventname="nsfw_update"
                    )
                ):
                    changes.append("nsfw_off")
                elif (
                    not before.is_nsfw()
                    and after.is_nsfw()
                    and self.is_logging(
                    ...
                    changes.append("nsfw_on")
                if (
                    (before.topic != after.topic)
                    and not (before.topic == "" and after.topic == None)
                    and self.is_logging(
                        after.guild, channel=after, eventname="channel_desc_update"
                    )
                ):
            ...
            if len(changes):
                ...
                e = discord.Embed(
                    ...
                    description=(
                        ...
                        if "TitleUpdate" in changes
                        else ""
                    )
                    + (
                        f"**Topic:** {before.topic} > {after.topic}\n"
                        if "TopicUpdate" in changes
                        else ""
                    )
                    + (
                        f"**NSFW:** {'Now NSFW' if 'nsfw_on' in changes else 'No longer NSFW'}\n"
                        if "nsfw_on" in changes or "nsfw_off" in changes
                        else ""
                    )
    ...
    async def on_invite_delete(self, invite: discord.Invite):
        if not isinstance(invite.guild, discord.Guild):
            return
        if not self.is_logging(
            invite.guild, channel=invite.channel, member=None, eventname="invite_create"
        ):
            return
        audit = await get_alog_entry(
            invite.channel, type=discord.AuditLogAction.invite_delete
        )
        log = self.get_log(invite.guild)
        e = discord.Embed(
    ...
    async def on_guild_role_create(self, role: discord.Role):
        if not self.is_logging(role.guild, eventname="guild_role_create"):
            return
        ...
        return await self.log(
            ...
            occurredAt=round(time.time()),
            guild=role.guild.id,
            content={"username": audit.user.id, "name": role.name, "id": role.id},
        )

    @commands.Cog.listener()
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        if not self.is_logging(after.guild, eventname="roles"):
            return
        audit = await get_alog_entry(before, type=discord.AuditLogAction.role_update)
        if before.name == after.name and before.colour == after.colour:
            return
