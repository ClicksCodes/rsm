...
class Emergency(commands.Cog):
    ...
    async def lock(self, ctx):
        ...
        if isinstance(await self.handlers.checkPerms(ctx, m, "manage_guild", self.emojis().commands.lock, "lock a channel"), Failed):
            return
    ...
    async def unlock(self, ctx):
        m = await ctx.send(embed=loading_embed)
        if isinstance(await self.handlers.checkPerms(ctx, m, "manage_guild", self.emojis().commands.lock, "unlock a channel"), Failed):
            return
